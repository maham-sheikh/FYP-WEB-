...
const UnverifiedBusinesses = () => {
    ...
    const fetchUnverifiedBusinesses = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/business/unverified"
        );
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(term);
    const filtered = unverifiedBusinessesData.filter(
      (business) =>
  ...
  const handleApprove = async (businessId) => {
    try {
      await axios.put("http://localhost:8000/api/business/approve", {
        id: businessId,
      });
      ...
      setUnverifiedBusinessesData(
        updatedList.filter((business) => business.status !== "approved")
      );
  ...
  const handleReject = async (businessId) => {
    try {
      await axios.put("http://localhost:8000/api/business/reject", {
        id: businessId,
      });
                 ...
                 {/* <td>{business.password}</td> */}
                  ...
                  <td>
                    {business.status === "pending" && (
                      <>
                        <button
                          className="business_button approve"
                          onClick={() => handleApprove(business.id)}
                        >
                          Approve
                        </button>
                        <button
                          className="business_button reject"
