...
const approveBusiness = async (req, res) => {
  ...
  try {
    await db.query(`UPDATE vendors SET status = 'approved' WHERE id = ?`, [id]);

    const [vendor] = await db.query(`SELECT email FROM vendors WHERE id = ?`, [
      id,
    ]);
    ...
    if (vendor.length > 0) {
      ...
      const text =
        "Congratulations! Your business has been approved. You can now log in to the Findigo app.";
...
const rejectBusiness = async (req, res) => {
  ...
  try {
    await db.query(`UPDATE vendors SET status = 'rejected' WHERE id = ?`, [id]);
    ...
    if (vendor.length > 0) {
      ...
      const text =
        "We regret to inform you that your business application has been rejected. Please contact support for further details.";
